...
| `TableRendererProvider` | El build reveló que `6fca4da3e` lo referenciaba pero no lo había versionado; la definición neutral exacta seguía preservada en `stash@{0}`. | Se recuperó durante la migración y se retiró al consolidar los dos launchers explícitos; el contrato vigente es `TableRenderer`. |
...
| `TableRendererRegistry` dinámico | Dejó de ser necesario al separar los dos JAR. | Retirado junto al descriptor `ServiceLoader`; cada launcher tiene wiring explícito y probado. |
...
- Completado: eliminado `TableRendererRegistry` al quedar consolidados los dos ejecutables independientes.
