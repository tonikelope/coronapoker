...
- **Zero-knowledge & cost.** The proofs reveal nothing about the permutation or the locks. All proving and verifying runs in the **background during betting, single-threaded**, with no impact on the dealing animation or CPU.
...
A passive MITM cannot complete the handshake without the password: the shared secret is sealed by HMAC-SHA512(`password \|\| dh_secret`), and a wrong password yields a wrong key on both sides, so every subsequent command fails HMAC verification on receipt.
