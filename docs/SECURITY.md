...
The first time a pubkey is observed for a given NFC-canonical nick the resolver pins it ([`TOFUResolver.java`](../src/main/java/com/tonikelope/coronapoker/TOFUResolver.java)). Every admission path accepts only `NEW` or byte-identical `MATCH`; `CHANGED` and resolver errors preserve the previous key and verification state and close the channel before admission. Legacy Unicode aliases are migrated only when unambiguous, and conflicting aliases fail closed. The reserved `CoronaBot$` namespace cannot be claimed by a remote human. A `Mode.IDENTITY` identicon dialog renders the same deterministic mosaic over `SHA-256(pubkey)` for OOB comparison. Clicking "verify" upgrades the entry from TOFU to "user-verified", remembered across sessions.
