 ...
 * Validated against algebraic properties — group order L, scalar commutativity
 * — in EdwardsPointTest. Ristretto encode/decode is layered on top separately.
 *
...
public final class EdwardsPoint {
    ...
    /**
     * Order of the prime-order subgroup: L = 2^252 +
    ...
    /**
     * Lazily-memoized 4-bit window table ({@code 0..15 · this}) used by
     * {@link #scalarMul}. The instance is immutable, so for a point
     * scalar-multiplied many times — above all the constant {@link #BASE}, the
     * {@code k·B} commitments and DLEQ bases — the 15-addition table is built
     * once and reused. {@code volatile} for safe publication across the
     * background SRA verifier threads.
     */
    private volatile EdwardsPoint[] windowTable;

    /**
     * Lazily-memoized odd-multiple table ({@code 1,3,5,…,15 · this}) for
     * {@link #scalarMul}'s width-5 signed-window (wNAF) ladder. Eight points,
     * built with one doubling and seven additions — lighter than
     * {@link #windowTable}'s fifteen — and, like it, memoized so a reused point
     * (the constant {@link #BASE}, {@code k·B} commitments, DLEQ bases) builds
     * it once, not per call. {@code volatile} for safe publication across the
     * background SRA verifier threads.
     */
    private volatile EdwardsPoint[] nafTable;

    /**
     * Lazily-memoized canonical Ristretto255 encoding of this point (see
     * {@link Ristretto255#encode}). Same pure-cache pattern as
     * {@link #windowTable}: the instance is immutable, so its canonical
     * encoding is a constant — computed at most once, and seeded for free by
     * {@link Ristretto255#decode} (RFC 9496 guarantees the round-trip), so wire
     * points re-encode without paying the sqrt-ratio exponentiation. Holds a
     * private copy; {@code encode} hands out clones. {@code volatile} for safe
     * publication across the background SRA verifier threads.
     */
    ...
    /**
     * Scalar multiplication s*P via a width-5 signed-window (wNAF)
     * double-and-add, MSB to LSB. s must be >= 0. The result is the same group
     * element as any correct s*P (bit-identical once encoded), validated
     * against a naive double-and-add over random inputs in
     * ScalarMulWnafDiffTest.
     */
    ...
    /**
     * Memoized 4-bit window table {@code [0..15]·this} for {@link #scalarMul}
     * (see {@link #windowTable}).
     */
     ...
     * (2j+1)·this} for {@code j = 0..7}, i.e. {@code 1,3,5,…,15 · this}. Built
     * with one doubling ({@code 2·this}) and seven additions.
     */
    ...
    /**
     * Multi-scalar multiplication {@code Σ scalars[i]·points[i]} via Straus'
     * method: ONE shared doubling ladder over all points (4-bit windows),
     * instead of one independent {@link #scalarMul} per point. For {@code n}
     * points this does ~256 doublings total (shared) instead of ~256·n, the
     * dominant cost of every proof's {@code Σ wᵢ·Pᵢ}. Result is bit-identical
     * to the naive sum (validated by fuzzing in MultiScalarMulTest). Scalars
     * are reduced mod L; not constant-time.
     */
