...
/**
 * Swing/Java2D card flip animation (replaces the pre-generated gifsicle spin
 * GIFs).
 *
 * The flip is a 180-degree 3D rotation around the vertical axis with
 * PERSPECTIVE (the card reads as a trapezoid, not a flat squeeze). It's solved
 * with a per-pixel inverse warp: each output pixel's exact source coordinate in
 * the JPG is computed with a closed-form formula and sampled with bilinear
 * interpolation — no replicated "strips", so it stays as sharp as the native
 * JPG.
 *
 * Frames are generated at the monitor's PHYSICAL HiDPI resolution so the system
 * never upscales them, and the card is drawn at or below native JPG size.
 *
 * The resulting frames are wrapped in a {@link PreRenderedGif} and played by
 * the existing catch-up engine (same timing, hooks and sync as regular GIFs).
 *
 * @author tonikelope
 */
public class CardFlipAnimator {
    ...
    /**
     * HiDPI scale of the primary monitor (1.0, 1.25, 2.0...). Frames are
     * generated at this density so they land 1:1 in physical pixels.
     *
     * @return clamped scale factor in [1.0, 3.0], or 1.0 if it can't be
     * determined
     */
     ...
     * @param valor_palo card key, e.g. "A_P" (never "trasera" — the back is
     * loaded separately)
     * @param card_w_logical logical width of the static card (Card.CARD_WIDTH)
     * @param card_h_logical logical height of the static card
     * (Card.CARD_HEIGHT)
     * @param corner_logical logical corner radius (Card.CARD_CORNER)
     * @param duration_ms total flip duration
     * @param num_frames number of frames to generate
     * @param zoom zoom factor over the static card size (1.0 = pixel-perfect
     * alignment; &gt;1.0 draws the card larger for a zoom-in effect)
     * @param top_half compact view: flip only the card's TOP HALF (matching the
     * split static view) instead of the whole card
    ...
    /**
     * Logical canvas width for the flip animation (used to size the label). The
     * margin is symmetric and even around the drawn card (canvas = drawn +
     * 2*margin) so overlay centering on the static card lands on an integer
     * pixel. With zoom=1 this matches the original pixel-perfect canvas
     * exactly.
     */
    ...
    /**
     * Invalidates the source cache if the deck or the selected card back
     * changed.
     */
    ...
    /**
     * Global card back (game deck or mod), rounded at native resolution and
     * cached.
     */
    ...
    /**
     * Crops the TOP HALF at native resolution (first h/2 pixel rows, unscaled)
     * for the compact flip view — the same crop the split static card shows.
     * Copies pixels (not getSubimage) so the cached source raster isn't shared,
     * and rounds the new bottom corners at the crop line to the same native
     * radius rounded() used for the top ones, so all four corners of the split
     * card match, just like on the static card.
     */
    ...
    /**
     * Per-pixel inverse perspective warp: renders the card rotated angleDeg
     * degrees (0 = back facing forward, 180 = front facing forward) into a
     * BufferedImage.
     */
    ...
    private static int sampleBilinear(int[] p, int w, int h, double fx, double fy) {
        ...
        int ri = (int) Math.round(R / A);
        if (ri > 255) {
            ri = 255;
        }
        int gi = (int) Math.round(G / A);
        if (gi > 255) {
            gi = 255;
        }
        int bi = (int) Math.round(B / A);
        if (bi > 255) {
            bi = 255;
        }
