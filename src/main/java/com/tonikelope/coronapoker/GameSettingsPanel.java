...
/**
 * "Game settings" tab content as a JPanel (page of the unified dialog): game
 * RULES on the left (no subpanel) and BLINDS on the right (in a titled
 * subpanel). Built READ-ONLY for clients. All blind logic (money-sensitive)
 * comes from the old EditBlindsDialog. Applied via {@link #applyToGame()}
 * (triggered by the unified dialog's SAVE button); appearance and audio apply
 * live, rules/blinds only on save.
 *
 * @author tonikelope
 */
public class GameSettingsPanel extends javax.swing.JPanel {
    ...
    /**
     * Whether the Game tab has unsaved changes. Used by the dialog to confirm
     * before discarding on cancel.
     *
     * @return {@code true} if any control differs from its value when the tab
     * was opened
     */
    ...
    /**
     * Applies pending Game-tab changes (rules + blinds) to the live game.
     * Triggered by the unified dialog's SAVE button; does nothing when
     * {@link #isReadOnly()}.
     */
