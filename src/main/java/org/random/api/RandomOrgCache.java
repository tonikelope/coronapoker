...
/**
 * Precache class for frequently used requests. * * ** WARNING ** * Instances of
 * this class should only be obtained using a RandomOrgClient's * createCache()
 * methods. * * This class strives to keep a Queue of response results populated
 ...
 * intervals. * * @param <T> return array type, e.g., int[] *
 *
 */
public class RandomOrgCache<T> {
     ...
     * * ** WARNING ** * Should only be called by RandomOrgClient's
     * createCache() methods. * * @param requestFunction function used to send
     * supplied request to server.
     *
     ** @param processFunction function to process result of requestFunction
     * into expected output.
     ** @param request request to send to server via requestFunction.
     ** @param cacheSize number of request responses to try maintain.
     ** @param bulkRequestNumber if request is set to be issued in bulk, number
     * of result sets in a bulk request, else 0.
     ** @param requestNumber if request is set to be issued in bulk, number of
     * results in a single request, else 0.
     ** @param singleRequestSize in bits for adjusting bulk requests if bits
     * are in short supply on the server.
     *
     */
    ...
    /**
     * Keep issuing requests to server until Queue is full. When Queue is full
     * if requests * are being issued in bulk, wait until Queue has enough space
     * to accommodate all of a * bulk request before issuing a new request,
     * otherwise issue a new request every time * an item in the Queue has been
     * consumed. * * Note that requests to the server are blocking, i.e., only
     * one request will be issued by * the cache at any given time.
     *
     */
     ...
     ** <p>
     ** This state can be changed with <code>stop()</code> and
     * <code>resume()</code>. * * @see #stop()
     *
     ** @see #resume() * * @return <code>true</code> if cache is currently not
     * re-populating itself.
     *
     */
    ...
    /**
     * Get next response. * * @return next appropriate response for the request
     * this RandomOrgCache represents * or if Queue is empty throws a
     ...
     ** <p>
     ** Note: if the cache is paused or no more randomness is available from
     * the server this call can result in a dead lock. * * @see #isPaused()
     *
     * *
     * @return next appropriate response for the request this RandomOrgCache
     * represents * * @throws InterruptedException if any thread interrupted the
     ...
     ** <p>
     ** This essentially returns how often <code>get()</code> may be called
     * without a cache refill, * or <code>getOrWait()</code> may be called
     * without blocking. * * @return current number of cached results
     *
     */
    ...
    /**
     * Get number of bits used by this cache. * * @return number of used bits
     *
     */
    ...
    /**
     * Get number of requests used by this cache. * * @return number of used
     * requests
     *
     */
