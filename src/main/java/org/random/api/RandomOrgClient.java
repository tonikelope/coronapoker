...
/**
 * RandomOrgClient main class through which API functions are accessed. * * This
 * class provides either serialized or unserialized (determined on class
 * creation) * access to both the signed and unsigned methods of the RANDOM.ORG
 * API. These are * threadsafe and implemented as blocking remote procedure
 * calls. * * If requests are to be issued serially a background Thread will
 * maintain a Queue of * requests to process in sequence. * * The class also
 * provides access to creation of a convenience class, RandomOrgCache, * for
 * precaching API responses when the request is known in advance. * * This class
 * will only allow the creation of one instance per API key. If an instance * of
 * this class already exists for a given key, that instance will be returned
 * instead * of a new instance. * * This class obeys most of the guidelines set
 * forth in https://api.random.org/guidelines * All requests respect the
 * server's advisoryDelay returned in any responses, or use * DEFAULT_DELAY if
 * no advisoryDelay is returned. If the supplied API key is paused, i.e., * has
 * exceeded its daily bit/request allowance, this implementation will back off
 * until * midnight UTC. * * @see https://api.random.org/
 *
...
public class RandomOrgClient {
    ...
    /**
     * Ensure only one instance of RandomOrgClient exists per API key. Create a
     * new instance if the * supplied key isn't already known, otherwise return
     * the previously instantiated one. * New instance will have a
     * blockingTimeout of 24*60*60*1000 milliseconds, i.e., 1 day, a httpTimeout
     * of * 120*1000 milliseconds, and will issue serialized requests.
     *
     * *
     * @param apiKey of instance to create/find, obtained from RANDOM.ORG, see:
     * https://api.random.org/api-keys * * @return new instance if instance
     * doesn't already exist for this key, else existing instance.
     *
     */
     ...
     * Ensure only one instance of RandomOrgClient exists per API key. Create a
     * new instance if the * supplied key isn't already known, otherwise return
     * the previously instantiated one. * * @param apiKey of instance to
     * create/find, obtained from RANDOM.ORG, see:
     * https://api.random.org/api-keys
     *
     ** @param blockingTimeout maximum time in milliseconds to wait before
     * being allowed to send a request. * Note this is a hint not a guarantee.
     * Be advised advisory delay from server must always be obeyed. * Supply a
     * value of -1 to allow blocking forever. (default 24*60*60*1000, i.e., 1
     * day).
     ** @param httpTimeout maximum time in milliseconds to wait for the server
     * response to a request. (default 120*1000).
     ** @param serialized determines whether or not requests from this instance
     * will be added to a Queue and *	issued serially or sent when received,
     * obeying any advisory delay (default true). * * @return new instance if
     * instance doesn't already exist for this key, else existing instance.
     *
     */
    ...
    /**
     * Constructor. Initialize class and start serialized request sending Thread
     * running as a daemon if applicable. * * @param apiKey of instance to
     * create/find, obtained from RANDOM.ORG, see:
     * https://api.random.org/api-keys
     *
     ** @param blockingTimeout maximum time in milliseconds to wait before
     ...
     ** @param serialized determines whether or not requests from this instance
     * will be added to a Queue and *	issued serially or sent when received,
     * obeying any advisory delay (default true).
     *
     */
    ...
    /**
     * Request and return an array of true random integers within a user-defined
     * range from the server. * See:
     * https://api.random.org/json-rpc/1/basic#generateIntegers * * @param n how
     * many random integers you need. Must be within the [1,1e4] range.
     *
     ** @param min the lower boundary for the range from which the random
     * numbers will be picked. Must be within the [-1e9,1e9] range.
     ** @param max the upper boundary for the range from which the random
     * numbers will be picked. Must be within the [-1e9,1e9] range. * * @return
     ...
     * has been exceeded.
     ** @throws RandomOrgBadHTTPResponseException if a HTTP 200 OK response not
     * received.
     ...
     ** @throws MalformedURLException in the unlikely event something goes
     * wrong with URL creation. @see java.net.MalformedURLException
     ...
     ** @param max the upper boundary for the range from which the random
     * numbers will be picked. Must be within the [-1e9,1e9] range.
     ** @param replacement specifies whether the random numbers should be
     * picked with replacement. *	If True the resulting numbers may contain
     * duplicate values, otherwise the numbers will all be unique (default
     * True). * * @return array of random integers. * * @throws
     * RandomOrgSendTimeoutException blocking timeout is exceeded before the
     * request can be sent.
     ** @throws RandomOrgKeyNotRunningError API key has been stopped.
     ** @throws RandomOrgInsufficientRequestsError API key's server requests
     * allowance has been exceeded.
    ...
    /**
     * Request and return a list (size n) of true random decimal fractions, from
     * a uniform distribution across * the [0,1] interval with a user-defined
     * number of decimal places from the server. * See:
     * https://api.random.org/json-rpc/1/basic#generateDecimalFractions
     *
     * *
     * @param n how many random decimal fractions you need. Must be within the
     * [1,1e4] range.
     ** @param decimalPlaces the number of decimal places to use. Must be
     * within the [1,20] range. * * @return array of random doubles. * * @throws
     * RandomOrgSendTimeoutException blocking timeout is exceeded before the
     * request can be sent.
     ** @throws RandomOrgKeyNotRunningError API key has been stopped.
     ** @throws RandomOrgInsufficientRequestsError API key's server requests
     ...
     * picked with replacement. *	If True the resulting numbers may contain
     * duplicate values, otherwise the numbers will all be unique (default
     * True). * * @return array of random doubles. * * @throws
     * RandomOrgSendTimeoutException blocking timeout is exceeded before the
     * request can be sent.
     ** @throws RandomOrgKeyNotRunningError API key has been stopped.
     ** @throws RandomOrgInsufficientRequestsError API key's server requests
    ...
    /**
     * Request and return a list (size n) of true random numbers from a Gaussian
     * distribution (also known as a * normal distribution). The form uses a
     * Box-Muller Transform to generate the Gaussian distribution from *
     * uniformly distributed numbers. See:
     * https://api.random.org/json-rpc/1/basic#generateGaussians * * @param n
     * how many random numbers you need. Must be within the [1,1e4] range.
     *
     ** @param mean the distribution's mean. Must be within the [-1e6,1e6]
     * range.
     ** @param standardDeviation the distribution's standard deviation. Must be
     * within the [-1e6,1e6] range.
     ** @param significantDigits the number of significant digits to use. Must
     * be within the [2,20] range. * * @return array of true random doubles from
     * a Gaussian distribution. * * @throws RandomOrgSendTimeoutException
     * blocking timeout is exceeded before the request can be sent.
     ** @throws RandomOrgKeyNotRunningError API key has been stopped.
     ** @throws RandomOrgInsufficientRequestsError API key's server requests
     * allowance has been exceeded.
    ...
    /**
     * Request and return a list (size n) of true random unicode strings from
     * the server. * See:
     * https://api.random.org/json-rpc/1/basic#generateStrings * * @param n how
     * many random strings you need. Must be within the [1,1e4] range.
     *
     ** @param length the length of each string. Must be within the [1,20]
     * range. All strings will be of the same length.
     ** @param characters a string that contains the set of characters that are
     * allowed to occur in the random strings. *	The maximum number of
     * characters is 80. * * @return array of random Strings. * * @throws
     * RandomOrgSendTimeoutException blocking timeout is exceeded before the
     * request can be sent.
     ** @throws RandomOrgKeyNotRunningError API key has been stopped.
     ** @throws RandomOrgInsufficientRequestsError API key's server requests
     ...
     * allowed to occur in the random strings. *	The maximum number of
     * characters is 80.
     ** @param replacement specifies whether the random strings should be
     * picked with replacement. If True the resulting *	list of strings may
     * contain duplicates, otherwise the strings will all be unique (default
     * True). * * @return array of random Strings. * * @throws
     * RandomOrgSendTimeoutException blocking timeout is exceeded before the
     * request can be sent.
     ** @throws RandomOrgKeyNotRunningError API key has been stopped.
     ** @throws RandomOrgInsufficientRequestsError API key's server requests
    ...
    /**
     * Request and return a list (size n) of version 4 true random Universally
     * Unique IDentifiers (UUIDs) in accordance * with section 4.4 of RFC 4122,
     * from the server. See:
     * https://api.random.org/json-rpc/1/basic#generateUUIDs * * @param n how
     * many random UUIDs you need. Must be within the [1,1e3] range. * * @return
     * array of random UUIDs. * * @throws RandomOrgSendTimeoutException blocking
     * timeout is exceeded before the request can be sent.
     *
     ** @throws RandomOrgKeyNotRunningError API key has been stopped.
     ** @throws RandomOrgInsufficientRequestsError API key's server requests
     * allowance has been exceeded.
    ...
    /**
     * Request and return a list (size n) of Binary Large OBjects (BLOBs) as
     * unicode strings * containing true random data from the server. See:
     * https://api.random.org/json-rpc/1/basic#generateBlobs * * @param n how
     * many random blobs you need. Must be within the [1,100] range.
     *
     ** @param size the size of each blob, measured in bits. Must be within the
     * [1,1048576] range and must be divisible by 8. * * @return array of random
     * blobs as Strings. * * @throws RandomOrgSendTimeoutException blocking
     * timeout is exceeded before the request can be sent.
     ** @throws RandomOrgKeyNotRunningError API key has been stopped.
     ** @throws RandomOrgInsufficientRequestsError API key's server requests
     * allowance has been exceeded.
     ...
     *
     ** @param size the size of each blob, measured in bits. Must be within the
     * [1,1048576] range and must be divisible by 8.
     ** @param format specifies the format in which the blobs will be returned.
     * Values allowed are * BLOB_FORMAT_BASE64 and BLOB_FORMAT_HEX (default
     * BLOB_FORMAT_BASE64). * * @return array of random blobs as Strings.
     *
     * @throws RandomOrgSendTimeoutException blocking timeout is exceeded before
    ...
    /**
     * Request a list (size n) of true random integers within a user-defined
     * range from the server. Returns a * dictionary object with the parsed
     * integer list mapped to 'data', the original response mapped to 'random',
     * * and the response's signature mapped to 'signature'. See:
     * https://api.random.org/json-rpc/1/signing#generateSignedIntegers
     *
     * *
     * @param n how many random integers you need. Must be within the [1,1e4]
     * range.
     ** @param min the lower boundary for the range from which the random
     * numbers will be picked. Must be within the [-1e9,1e9] range.
     ** @param max the upper boundary for the range from which the random
     * numbers will be picked. Must be within the [-1e9,1e9] range. * * @return
     * HashMap with "random": random JsonObject, "signature": signature String,
     * "data": random int[] * * @throws RandomOrgSendTimeoutException blocking
     * timeout is exceeded before the request can be sent.
     ** @throws RandomOrgKeyNotRunningError API key has been stopped.
     ** @throws RandomOrgInsufficientRequestsError API key's server requests
     * allowance has been exceeded.
     ...
     * picked with replacement. *	If True the resulting numbers may contain
     * duplicate values, otherwise the numbers will all be unique (default
     * True). * * @return HashMap with "random": random JsonObject, "signature":
     * signature String, "data": random int[] * * @throws
     * RandomOrgSendTimeoutException blocking timeout is exceeded before the
     * request can be sent.
     ** @throws RandomOrgKeyNotRunningError API key has been stopped.
     ** @throws RandomOrgInsufficientRequestsError API key's server requests
     ...
     * decimal places from the server. Returns a dictionary object with the
     * parsed decimal * fraction list mapped to 'data', the original response
     * mapped to 'random', and the response's signature mapped to * 'signature'.
     * See:
     * https://api.random.org/json-rpc/1/signing#generateSignedDecimalFractions
     *
     * * * @param n how many random decimal fractions you need. Must be within
     * the [1,1e4] range.
     ** @param decimalPlaces the number of decimal places to use. Must be
     * within the [1,20] range. * * @return HashMap with "random": random
     * JsonObject, "signature": signature String, "data": random double[]
     *
     * @throws RandomOrgSendTimeoutException blocking timeout is exceeded before
     * the request can be sent.
     ** @throws RandomOrgKeyNotRunningError API key has been stopped.
     ...
     * duplicate values, otherwise the numbers will all be unique (default
     * True). * * @return HashMap with "random": random JsonObject, "signature":
     * signature String, "data": random double[] * * @throws
     * RandomOrgSendTimeoutException blocking timeout is exceeded before the
     * request can be sent.
     ** @throws RandomOrgKeyNotRunningError API key has been stopped.
     ** @throws RandomOrgInsufficientRequestsError API key's server requests
     ...
     * distribution (also known as a normal distribution). * The form uses a
     * Box-Muller Transform to generate the Gaussian distribution from uniformly
     * distributed numbers. * Returns a dictionary object with the parsed random
     * number list mapped to 'data', the original response mapped to 'random', *
     * and the response's signature mapped to 'signature'. See:
     * https://api.random.org/json-rpc/1/signing#generateSignedGaussians
     *
     * *
     * @param n how many random numbers you need. Must be within the [1,1e4]
     * range.
     ** @param mean the distribution's mean. Must be within the [-1e6,1e6]
     * range.
     ** @param standardDeviation the distribution's standard deviation. Must be
     * within the [-1e6,1e6] range.
     ** @param significantDigits the number of significant digits to use. Must
     * be within the [2,20] range. * * @return HashMap with "random": random
     * JsonObject, "signature": signature String, "data": random double[]
     *
     * @throws RandomOrgSendTimeoutException blocking timeout is exceeded before
     * the request can be sent.
    ...
    /**
     * Request a list (size n) of true random strings from the server. Returns a
     * dictionary object with the parsed random * string list mapped to 'data',
     * the original response mapped to 'random', and the response's signature
     * mapped to 'signature'. * See:
     * https://api.random.org/json-rpc/1/signing#generateSignedStrings
     *
     * *
     * @param n how many random strings you need. Must be within the [1,1e4]
     * range.
     ** @param length the length of each string. Must be within the [1,20]
     * range. All strings will be of the same length.
     ** @param characters a string that contains the set of characters that are
     * allowed to occur in the random strings. *	The maximum number of
     * characters is 80. * * @return HashMap with "random": random JsonObject,
     * "signature": signature String, "data": random String[] * * @throws
     * RandomOrgSendTimeoutException blocking timeout is exceeded before the
     * request can be sent.
     ** @throws RandomOrgKeyNotRunningError API key has been stopped.
     ** @throws RandomOrgInsufficientRequestsError API key's server requests
     ...
     * contain duplicates, otherwise the strings will all be unique (default
     * True). * * @return HashMap with "random": random JsonObject, "signature":
     * signature String, "data": random String[] * * @throws
     * RandomOrgSendTimeoutException blocking timeout is exceeded before the
     * request can be sent.
     ** @throws RandomOrgKeyNotRunningError API key has been stopped.
     ** @throws RandomOrgInsufficientRequestsError API key's server requests
     ...
     * the server. Returns a dictionary object with the parsed random UUID list
     * mapped * to 'data', the original response mapped to 'random', and the
     * response's signature mapped to 'signature'. * See:
     * https://api.random.org/json-rpc/1/signing#generateSignedUUIDs * * @param
     * n how many random UUIDs you need. Must be within the [1,1e3] range.
     *
     * *
     * @return HashMap with "random": random JsonObject, "signature": signature
     * String, "data": random UUID[] * * @throws RandomOrgSendTimeoutException
     * blocking timeout is exceeded before the request can be sent.
     ** @throws RandomOrgKeyNotRunningError API key has been stopped.
     ** @throws RandomOrgInsufficientRequestsError API key's server requests
     * allowance has been exceeded.
     ...
     * random data from the server. Returns a * dictionary object with the
     * parsed random BLOB list mapped to 'data', the original response mapped to
     * 'random', * and the response's signature mapped to 'signature'. See:
     * https://api.random.org/json-rpc/1/signing#generateSignedBlobs * * @param
     * n how many random blobs you need. Must be within the [1,100] range.
     *
     ** @param size the size of each blob, measured in bits. Must be within the
     * [1,1048576] range and must be divisible by 8. * * @return HashMap with
     * "random": random JsonObject, "signature": signature String, "data":
     * random String[] * * @throws RandomOrgSendTimeoutException blocking
     * timeout is exceeded before the request can be sent.
     ** @throws RandomOrgKeyNotRunningError API key has been stopped.
     ** @throws RandomOrgInsufficientRequestsError API key's server requests
     * allowance has been exceeded.
     ...
     ** @param format specifies the format in which the blobs will be returned.
     * Values allowed are * BLOB_FORMAT_BASE64 and BLOB_FORMAT_HEX (default
     * BLOB_FORMAT_BASE64). * * @return HashMap with "random": random
     * JsonObject, "signature": signature String, "data": random String[]
     *
     * @throws RandomOrgSendTimeoutException blocking timeout is exceeded before
     * the request can be sent.
     ** @throws RandomOrgKeyNotRunningError API key has been stopped.
    ...
    /**
     * Verify the signature of a response previously received from one of the
     * methods in the Signed API with the * server. This is used to examine the
     * authenticity of numbers. Return True on verification success. * See:
     * https://api.random.org/json-rpc/1/signing#verifySignature * * @param
     * random the random field from a response returned by RANDOM.ORG through
     * one of the Signed API methods.
     *
     ** @param signature the signature field from the same response that the
     * random field originates from. * * @return verification success. * @throws
     * RandomOrgSendTimeoutException blocking timeout is exceeded before the
     * request can be sent.
     ** @throws RandomOrgKeyNotRunningError API key has been stopped.
     ** @throws RandomOrgInsufficientRequestsError API key's server requests
    ...
    /**
     * Get a RandomOrgCache to obtain random integers. The RandomOrgCache can be
     * polled for new results conforming to * the output format of the input
     * request. RandomOrgCache type is same as expected return value. * * @param
     ...
     * at any given time (default 20, minimum 2). * * @return
     * RandomOrgCache<int[]>
     *
     */
    ...
    /**
     * Get a RandomOrgCache to obtain random decimal fractions. The
     * RandomOrgCache can be polled for new results * conforming to the output
     * format of the input request. RandomOrgCache type is same as expected
     * return value. * * @param n how many random decimal fractions you need.
     * Must be within the [1,1e4] range.
     *
     ** @param decimalPlaces the number of decimal places to use. Must be
     * within the [1,20] range. * * @return RandomOrgCache<double[]>
     *
     */
    ...
    /**
     * Get a RandomOrgCache to obtain random numbers. The RandomOrgCache can be
     * polled for new results * conforming to the output format of the input
     * request. RandomOrgCache type is same as expected return value. * * @param
     * n how many random numbers you need. Must be within the [1,1e4] range.
     *
     ** @param mean the distribution's mean. Must be within the [-1e6,1e6]
     * range.
     ** @param standardDeviation the distribution's standard deviation. Must be
     * within the [-1e6,1e6] range.
     ** @param significantDigits the number of significant digits to use. Must
     * be within the [2,20] range. * * @return RandomOrgCache<double[]>
     *
     */
    ...
    /**
     * Get a RandomOrgCache to obtain random strings. The RandomOrgCache can be
     * polled for new results * conforming to the output format of the input
     * request. RandomOrgCache type is same as expected return value. * * @param
     ...
     ** @param characters a string that contains the set of characters that are
     * allowed to occur in the random strings. *	The maximum number of
     * characters is 80. * * @return RandomOrgCache<String[]>
     *
     */
    ...
    /**
     * Get a RandomOrgCache to obtain UUIDs. The RandomOrgCache can be polled
     * for new results conforming to the * output format of the input request.
     * RandomOrgCache type is same as expected return value. * * @param n how
     * many random UUIDs you need. Must be within the [1,1e3] range. * * @return
     * RandomOrgCache<UUID[]>
     *
     */
     ...
     * for new results conforming to the * output format of the input request.
     * RandomOrgCache type is same as expected return value. * * @param n how
     * many random UUIDs you need. Must be within the [1,1e3] range.
     *
     ** @param cacheSize number of result-sets for the cache to try to maintain
     * at any given time (default 10, minimum 2). * * @return
     * RandomOrgCache<UUID[]>
     *
     */
    ...
    /**
     * Get a RandomOrgCache to obtain random blobs. The RandomOrgCache can be
     * polled for new results conforming * to the output format of the input
     * request. RandomOrgCache type is same as expected return value. * * @param
     * n how many random blobs you need. Must be within the [1,100] range.
     *
     ** @param size the size of each blob, measured in bits. Must be within the
     * [1,1048576] range and must be divisible by 8. * * @return
     * RandomOrgCache<String[]>
     *
     */
    ...
    /**
     * Return the (estimated) number of remaining API requests available to the
     * client. If cached * usage info is older than
     * ALLOWANCE_STATE_REFRESH_SECONDS fresh info is obtained from server. * If
     * fresh info has to be obtained the following exceptions can be raised.
     *
     * *
     * @return number of requests remaining. * * @throws
     * RandomOrgSendTimeoutException blocking timeout is exceeded before the
     * request can be sent.
     ** @throws RandomOrgKeyNotRunningError API key has been stopped.
     ** @throws RandomOrgInsufficientRequestsError API key's server requests
    ...
    /**
     * Return the (estimated) number of remaining true random bits available to
     * the client. If cached * usage info is older than
     * ALLOWANCE_STATE_REFRESH_SECONDS fresh info is obtained from server. * If
     * fresh info has to be obtained the following exceptions can be raised.
     *
     * *
     * @return number of bits remaining. * * @throws
     * RandomOrgSendTimeoutException blocking timeout is exceeded before the
     * request can be sent.
     ** @throws RandomOrgKeyNotRunningError API key has been stopped.
     ** @throws RandomOrgInsufficientRequestsError API key's server requests
    ...
    /**
     * Issue a getUsage request to update bits and requests left. * * @throws
     * RandomOrgSendTimeoutException blocking timeout is exceeded before the
     * request can be sent.
     *
     ** @throws RandomOrgKeyNotRunningError API key has been stopped.
    ...
    /**
     * Add generic request parameters and API key to custom request. * * @param
     * params custom parameters to generate request around.
     *
     ** @param method to send request to. * * @return fleshed out JSON request.
     *
     */
    ...
    /**
     * Add generic request parameters to custom request. * * @param params
     * custom parameters to generate request around.
     *
     ** @param method to send request to. * * @return fleshed out JSON request.
     *
     */
    ...
    /**
     * Extracts int[] from JSON response. * * @param response JSON from which to
     * extract data. * * @return extracted int[].
     *
     */
    ...
    /**
     * Extracts double[] from JSON response. * * @param response JSON from which
     * to extract data. * * @return extracted double[].
     *
     */
    ...
    /**
     * Extracts String[] from JSON response. * * @param response JSON from which
     * to extract data. * * @return extracted String[].
     *
     */
    ...
    /**
     * Extracts UUID[] from JSON response. * * @param response JSON from which
     * to extract data. * * @return extracted UUID[].
     *
     */
    ...
    /**
     * Gets random data as separate from response JSON. * * @param response JSON
     * from which to extract data. * * @return JsonArray of random data.
     *
     */
     ...
     ** @param result to add signing data to. * * @return the passed in result
     * HasMap.
     *
     */
    ...
    /**
     * Gets verification response as separate from response JSON. * * @param
     * response JSON from which to extract verification response. * * @return
    ...
    /**
     * Send request as determined by serialized boolean. * * @param request JSON
     * to send. * * @return JsonObject response. * * @throws
     * RandomOrgSendTimeoutException blocking timeout is exceeded before the
     * request can be sent.
     *
     ** @throws RandomOrgKeyNotRunningError API key has been stopped.
    ...
    /**
     * Immediate call to server. Networking is run on a separate thread as
     * Android platform disallows networking on the main thread. * * @param
     * request JSON to send. * * @return JsonObject response. * * @throws
     * RandomOrgSendTimeoutException blocking timeout is exceeded before the
     * request can be sent.
     *
     ** @throws RandomOrgKeyNotRunningError API key has been stopped.
     ...
     *
     * *
     * @param request JSON to send. * * @return JsonObject response. * * @throws
     * RandomOrgSendTimeoutException blocking timeout is exceeded before the
     * request can be sent.
     ** @throws RandomOrgKeyNotRunningError API key has been stopped.
     ** @throws RandomOrgInsufficientRequestsError API key's server requests
    ...
    /**
     * Throw specific Exception types. * * @param e exception to throw.
     *
     * *
     * @throws RandomOrgSendTimeoutException blocking timeout is exceeded before
     * the request can be sent.
    ...
    /**
     * Core send request function. * * @param request JSON to send. * * @return
     * info on request success/response in a HashMap with one or other of the
     * following entries: *	"exception" : Exception - exception thrown, possible
     * exception types: *	RandomOrgSendTimeoutException *
     * RandomOrgKeyNotRunningError *	RandomOrgInsufficientRequestsError *
     * RandomOrgInsufficientBitsError * RandomOrgBadHTTPResponseException *
     * RandomOrgRANDOMORGError * RandomOrgJSONRPCError * MalformedURLException *
     * IOException *	"response"	: JsonObject - response
     *
     */
    ...
    /**
     * POST JSON to server and return JSON response. * * @param json request to
     * post. * * @return JSON response. * * @throws IOException @see
     * java.io.IOException
     *
     ** @throws MalformedURLException in the unlikely event something goes
     * wrong with URL creation. @see java.net.MalformedURLException
    ...
    private class UnserializedRunnable implements Runnable {
         ...
         */
        UnserializedRunnable(JsonObject request) {
            super();
        ...
        /**
         * @return data returned by network request - or null if not yet
         * arrived.
         */
