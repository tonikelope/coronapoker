 ...
 ** <p>
 ** This implements a true random number generator based on the RANDOM.ORG
 * service. * For more information see <code>https://api.random.org/</code>.
 ** <p>
 ** This class offers cached number retrieving from the random.org server over
 * the official API. * The cached numbers can be accessed bit wise by the
 * typical public methods of the random class. * If more bits are requested than
 * currently cached the methods will block until more data is * retrieved from
 * the server.
 ** <p>
 ** To use this class a official API key from
 * <code>https://api.random.org/api-keys</code> is required.
 ** <p>
 ** By default this caches a maximum of 512 bits in two blobs of 256 bits each.
 * * To optimize the cache for your needs you can setup the cache size (number
 * of blobs) and * cache bits (number of bits per blob) on creation of an
 * instance. * * @see https://random.org/
 *
 ** @see https://api.random.org/ * * @author Adam Wagenhaeuser
 * <adam@wag-web.de>
 *
 */
public class RandomOrgRandom extends Random {
    ...
    /**
     * Cache bits - number of bits to cache per blob. * * Must be within the
     * [1,1048576] range and must be divisible by 8. * This will be
     * automatically enforced (rounded up unless too large). * * Default is 256.
     *
     */
    private int cacheBits;

    /**
     * Cache size - number of blobs to cache. * * Cache size minimum is 2, which
     * will be automatically enforced. * Default size is 2.
     *
     */
    ...
    /**
     * The current blob from which random numbers are being pulled. * Once blob
     * is in use it is removed from the blob cache.
     *
     */
    ...
    /**
     * The RANDOM.ORG client used for the cache. * * @see #cache
     *
     */
    private RandomOrgClient client;

    /**
     * Left to right bit buffer for getting less than 8 bits. * * @see
     * #bitBufferLength
     *
     */
    ...
    /**
     * Number of bits currently stored in the <code>bitBuffer</code>. * * @see
     * #bitBuffer
     *
     */
     ...
     ** <p>
     ** The RANDOM.ORG library guarantees only one client is running per API
     * key at any * given time, so it's safe to create multiple
     * <code>RandomOrgRandom</code> classes * with the same API key if needed.
     *
     *
    ...
    /**
     * Create a new RandomOrgRandom instance for the given API key and cache
     * size.
     ** <p>
     ** This RandomOrgRandom needs an official API key for the RANDOM.ORG API.
     * * See https://api.random.org/api-keys for more details.
     ** <p>
     ** The <code>cacheSize</code> specifies the number of blobs maintained by
     * a background thread. * If you set the value too high it may consume much
     * of your daily allowance on the server. * It the value is too low your
     * application may block frequently. The default value is 2.
     ** <p>
     ** The <code>cacheBits</code> parameter specifies the number of bits per
     * blob in the cache. * If you set the value too high it may consume much of
     * your daily allowance on the server. * It the value is too low your
     * application will block frequently. The default value is 256.
     ** <p>
     ** The RANDOM.ORG library guarantees only one client is running per API
     * key at any * given time, so it's safe to create multiple
     * <code>RandomOrgRandom</code> classes * with the same API key if needed.
     ...
     ** <p>
     ** This RandomOrgRandom needs a RANDOM.ORG client instance for the
     * random.org API.
     ** <p>
     ** The RANDOM.ORG library guarantees only one client is running per API
     * key at any * given time, so it's safe to create multiple
     ...
     ** <p>
     ** This method gets the bit count still retrievable from the * server. If
     * this value is negative no more bits can be retrieved.
     ** <p>
     ** Note that this method will access a buffered value if that value is not
     * too * old, so the returned value can be different from the value on the
     * server.
     ** <p>
     ** Note that this value does not contain the local cached bits, so the
     * current * local available bit count can be larger than the return of this
     * method. * * @return the remaining bit quota * * @throws
     * RandomOrgSendTimeoutException @see {@link RandomOrgClient#getBitsLeft()}
     *
     ** @throws RandomOrgKeyNotRunningError @see
     * {@link RandomOrgClient#getBitsLeft()}
     ** @throws RandomOrgInsufficientRequestsError @see
     * {@link RandomOrgClient#getBitsLeft()}
     ** @throws RandomOrgInsufficientBitsError @see
     * {@link RandomOrgClient#getBitsLeft()}
     ** @throws RandomOrgBadHTTPResponseException @see
     * {@link RandomOrgClient#getBitsLeft()}
     ** @throws RandomOrgRANDOMORGError @see
     * {@link RandomOrgClient#getBitsLeft()}
     ** @throws RandomOrgJSONRPCError @see
     * {@link RandomOrgClient#getBitsLeft()}
     ** @throws MalformedURLException @see
     * {@link RandomOrgClient#getBitsLeft()}
     ...
     ** <p>
     ** This size can be specified in the constructor. * * @return the size of
     * the cache.
     *
     */
     ...
     ** <p>
     ** This size can be specified in the constructor. * * @return the number
     * of bits per blob in the cache.
     *
     */
    ...
    /**
     * Blocking implementation of {@link java.util.Random#nextInt(int)} using
     * RANDOM.ORG's RNG service. * * @see java.util.Random#nextInt(int)
     *
     * *
    ...
    /**
     * Blocking implementation of {@link java.util.Random#next(int)} using
     * RANDOM.ORG's RNG service. * * @see java.util.Random#next(int) * * @throws
     * NoSuchElementException if a number couldn't be retrieved.
     *
     */
    @Override
     ...
     ** <p>
     ** Note: for standard retrieval of bits use the <code>next(int)</code>
     * method.
     ** <p>
     ** <code>numBits</code> must be in the range of <code>1</code> to
     * <code>7</code> inclusive.
     ** <p>
     ** This method will return <code>numBits</code> bits as an 8 bit byte
     * value. * The random bits are stored in the least significant bits and the
     * most significant bits are filled with zeros. * All random numbers are in
     * the range of <code>0</code> to <code>2^numBits - 1</code> inclusive.
     ** <pre>
     ** Example getting 4 bits:
     **
     ** int ranBits = getSubBits(4) & 0x0F
     ** </pre> * * @param numBits number of bits to fetch * * @return a byte
     * containing the requested number of random bits * * @see #next(int)
     *
     * *
     * @throws IllegalArgumentException if the number of bits requested is not
    ...
    /**
     * Gets the next blob from the cache. * * @throws NoSuchElementException if
     * a blob couldn't be retrieved.
     *
     */
    ...
    /**
     * Return true if n is a power of 2. * * @param n to evaluate
     *
    ...
    /**
     * Return next power of 2 greater than n. * * @param n to evaluate
     *
    ...
    /**
     * Log2 of n. * * @param n to evaluate - should be a power of 2.
     *
     ** @return Log2 of n
     *
     */
